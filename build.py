...
TITLES = {
    ...
    "fusion-mcp": "Fusion MCP",
    "ibm-fusion": "IBM Fusion",
    "lazy-senior-dev": "lazy-senior-dev",
    "research-and-life": "Research and life",
}
...
def common(title: str, description: str, canonical: str, extra_head: str = "",
           og_image: str = SITE_CARD, collection: str = "") -> dict:
    # "Writing" goes to the index a reader is already inside. From a post or a collection page that
    # is the collection's own index, so the link narrows rather than throwing them back to the top.
    return {
        "title": esc(title),
        ...
        "writing_href": f"{BASE}/{collection}/" if collection else f"{BASE}/",
    }
...
def build_post(post: Post, ordered: list[Post], base_tpl: str) -> str:
    ...
    return fill(
        base_tpl,
        common(post.title, post.dek, canonical, og_image=card_image(post), collection=post.collection) | {"body": body},
    )



def build_collection(name: str, posts: list[Post], base_tpl: str) -> str:
    """The index for one collection. Cards for its own posts, newest first."""
    mine = [p for p in posts if p.collection == name]
    cards = "\n".join(render_card(p) for p in mine)
    onsite = [p for p in mine if not p.is_offsite]
    count = f"{len(mine)} post{'' if len(mine) == 1 else 's'}"
    if len(onsite) != len(mine):
        count += f" &middot; {len(onsite)} hosted here"
    # Reuses the classes the home page already styles rather than inventing a second card layout
    # that would drift away from it the first time either is touched.
    body = f"""<main id="main">
  <section class="wrap" style="padding-top:46px">
    <div class="eyebrow">{esc(count)}</div>
    <h1 class="post__title" style="margin:10px 0 12px">{esc(TITLES.get(name, name))}</h1>
    <p class="post__dek" style="margin-bottom:34px">{esc(COLLECTIONS[name])}</p>
    <div class="grid">
{cards}
    </div>
    <p style="margin-top:34px"><a href="{BASE}/">&larr; All writing</a></p>
  </section>
</main>"""
    return fill(
        base_tpl,
        common(TITLES.get(name, name), COLLECTIONS[name], f"{ORIGIN}{BASE}/{name}/", collection=name)
        | {"body": body},
    )
...
def build() -> None:
    ...
    for name in sorted({p.collection for p in posts}):
        target = OUT / name / "index.html"
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text(build_collection(name, posts, base_tpl), encoding="utf-8")
    ...
    check_internal_links(
        ...
        | {p.legacy_url for p in onsite}
        | {f"{BASE}/{c}/" for c in {p.collection for p in posts}},
    )
